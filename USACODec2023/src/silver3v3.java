...
//			pos0[j]=currPos;
		...
		int ans=Math.max(Math.max(L1, L2),Math.max(R2,Math.max(R1, ogTsHit)));
		
		// iterate
		for(int j=0;j<c;j++) {
			ans = Math.max(ans, pos0[j]+pos1L[c-1]-pos1L[j]);
			ans = Math.max(ans, pos0[j]+pos2L[c-1]-pos2L[j]);
			ans = Math.max(ans, pos0[j]+pos1R[c-1]-pos1R[j]);
			ans = Math.max(ans, pos0[j]+pos2R[c-1]-pos2R[j]);
		}
